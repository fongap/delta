...
from integrations.connectors.senders import _send_slack, _send_slack_interactive, _send_telegram
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        try:
            ...
            logger.warning(
                "python-telegram-bot not installed — `pip install delta[messaging]`"
            )
            return False
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        try:
            ...
            logger.warning(
                "slack-bolt not installed — `pip install delta[messaging]`"
            )
            return False
        ...
        async def _on_message(event, _say):
            ...
            if mapped is not None:
                ...
                # ...and a friendly channel/DM name so the GUI card shows "#delta-test", not "C…".
        ...
        # Button clicks on interactive prompts (action_id `delta_*`). Socket mode delivers these over
        # the same connection — no public endpoint, just "Interactivity" enabled in the Slack app.
        import re as _re

        @self._app.action(_re.compile(r"^delta_"))
