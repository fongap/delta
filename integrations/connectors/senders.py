...
def _send_slack(
    ...
    if err == "not_in_channel":
        err = "not_in_channel — invite @Delta to the channel in Slack, then retry"
    return SendResult(False, error=err)


def _slack_blocks(text: str, buttons) -> list[dict]:
    """A Block Kit message: a text section + a row of action buttons (action_id `delta_<i>`,
