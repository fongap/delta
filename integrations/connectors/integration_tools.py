...
from integrations.connectors.integration_helpers import (
    _attach,
    _clamp,
    _html_to_text,
    _now_ms,
...
from integrations.connectors.integration_github import (
    _github_call,
    _github_git_auth_args,
    _github_git_base,
    _run_git,
)
from integrations.connectors.integration_auth import (
    _atlassian_base,
    _basic_auth,
    _bearer_headers,
    _gitlab_api,
...
def _gmail_label_map(token: str) -> dict[str, str]:
    ...
    return {
        str(label.get("id") or ""): str(label.get("name") or "") for label in labels
    }
...
def _gmail_is_hidden(
    ...
    from integrations.connectors.gmail_accounts import sender_matches
...
def make_integration_tools(
    ...
    *,
    ...
    def outlook_search_messages(
        ...
        if err:
            return err
        params: dict[str, Any] = {"$top": max(1, min(int(max_results or 10), 20))}
        if query:
