...
from integrations.connectors.catalog_copy import about_for, access_for
from integrations.connectors.descriptors import get_descriptor, list_descriptors
from integrations.connectors.tool_defs import patch_tool_settings, tool_dicts
...
def _mcp_tokens_present(secrets: SecretStore, name: str) -> bool:
    ...
    from integrations.mcp.oauth import has_tokens
...
def disconnect_connector(secrets: SecretStore, name: str) -> dict[str, Any]:
    ...
    if profile.get("mode") == "mcp":
        ...
        from integrations.mcp import config as mcp_config
