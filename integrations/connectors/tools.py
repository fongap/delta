...
def _parse_or_coerce(target: str) -> tuple[str, str, str | None]:
    """parse_target, but a BARE channel name ('all-delta', '#general') coerces to
...
def _resolve_slack_channel(
    ...
    """'#all-delta' (a NAME the user said) → the team-qualified chat_id, via the
    ...
    from integrations.connectors.config import _slack_team_profiles
    from integrations.connectors.slack_directory import list_channels
    ...
    if not c.get("is_member"):
        return None, (
            f"found #{query}, but the bot isn't a member — invite @Delta to #{query} "
            "in Slack, then retry"
        )
    return chat_id, None
...
def _resolve_token(secrets: SecretStore, platform: str, chat_id: str) -> str | None:
    ...
    """
    if platform == "slack":
        from integrations.connectors.slack_addr import split
...
def make_send_message_tool(
    ...
    *,
    ...
    def send_message(target: str, text: str) -> dict[str, Any]:
        ...
        if platform == "slack":
            from integrations.connectors.attribution import sender_prefix
