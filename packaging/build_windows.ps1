...
# A running delta-server.exe (e.g. a prior sidecar/smoke test) locks the output exe and
...
Write-Host "==> [1/3] PyInstaller: bundling delta-server ($Triple)" -ForegroundColor Cyan
...
$Src = Join-Path $Here "dist\delta-server"
$Dst = Join-Path $BinDir "sidecar"
if (Test-Path $Dst) { Remove-Item -Recurse -Force $Dst }
# Clear any stale onefile binary from pre-onedir builds.
Remove-Item -Force (Join-Path $BinDir "delta-server-$Triple.exe") -ErrorAction SilentlyContinue
