...
python -m delta.connectors.cli status
    Show which platforms are configured (token present) + allowlist size.

python -m delta.connectors.cli fake [--user U1] [--allow U1]
...
python -m delta.connectors.cli send --target telegram:12345 --text "hi"
