...
old_header = "//! Credentials live in a private file and never cross the product IPC boundary.\n//! Connector workers own remote protocol details; this store owns product state."
new_header = "//! Product state and connector credentials live in one private authority file.\n//! Secrets never cross the product IPC boundary; connector workers own remote protocol details."
if text.count(old_header) != 1:
    raise SystemExit(f"header match count = {text.count(old_header)}")
text = text.replace(old_header, new_header, 1)

old_import = "use crate::ShadowReadError;"
new_import = "use crate::{durability::atomic_write_private, ShadowReadError};"
if text.count(old_import) != 1:
    raise SystemExit(f"import match count = {text.count(old_import)}")
text = text.replace(old_import, new_import, 1)

start = text.index("pub struct ApplicationStore {")
end = text.index("    pub fn connectors", start)
replacement = '''#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
connect_start = text.index("    pub fn connect(")
mutation_start = text.index(
    "        // Validate both authoritative files before mutating either one.",
    connect_start,
)
mutation_end = text.index(
    '        Ok(json!({"ok": true, "account": account}))',
    mutation_start,
)
connect_replacement = '''        let mut authority = self.read_authority()?;
        ...
        self.write_authority(&authority)?;
'''
text = text[:mutation_start] + connect_replacement + text[mutation_end:]

disconnect_start = text.index("    pub fn disconnect(")
mutation_start = text.index(
    "        // Validate both authoritative files before mutating either one.",
    disconnect_start,
)
...
disconnect_replacement = '''        let mut authority = self.read_authority()?;
        let removed = authority.state.connectors.remove(name).is_some();
        authority.secrets.remove(name);
        self.write_authority(&authority)?;
'''
text = text[:mutation_start] + disconnect_replacement + text[mutation_end:]

helper_start = text.index("#[cfg(unix)]\nfn restrict_private_file")
tests_start = text.index("#[cfg(test)]\nmod tests", helper_start)
text = text[:helper_start] + text[tests_start:]
...
mod tests {
    ...
    fn legacy_state_and_secrets_migrate_once_into_single_authority() {
        ...
        std::fs::write(&legacy_secrets, serde_json::to_vec_pretty(&secrets).unwrap()).unwrap();
...
insert_at = text.index("#[cfg(test)]\nmod tests")
cleanup = '''fn remove_if_exists(path: &Path) -> Result<(), ShadowReadError> {
...
'''
text = text[:insert_at] + cleanup + text[insert_at:]
