# delta GUI (React + Tauri)

A thin client of the delta server (OpenAI-compatible API + WS event/approval stream).
Same codebase runs in a browser (dev) and as the Delta desktop app.
...
```bash
bash scripts/setup_dev_env.sh   # → .venv (server + aisuite)
```
...
`VITE_DELTA_HTTP` / `VITE_DELTA_WS`). Start the server before Vite so the
