  ...
  return (
    <div data-testid="hubspot-detail">
      <div className="flex items-center gap-3.5 mb-5">
        <ConnectorBadge connector={c} size={44} title={t("connectors.brand.hubspot")} />
        <div className="min-w-0 flex-1">
          <h2 className="text-[20px] font-semibold tracking-tight leading-tight">
            {t("connectors.brand.hubspot")}
          </h2>
            ...
            {c.connected ? (
              <>
                ...
                <span data-testid="hubspot-status">
                  {t("connectors.portalCount", { n: portals.length })}
                </span>
              </>
            ) : (
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {t("connectors.hubspotNotConnectedBlurb")}
          </div>
        </div>
      )}
      ...
      {portals.length > 0 && (
        <>
          <div className={GRP_H + " !mt-0"}>{t("connectors.portals")}</div>
      ...
      <div className={FOOT + " mt-2"}>{t("connectors.hubspotPrivacyFootnote")}</div>
  ...
  return (
    <div className={ROW} data-testid={`hubspot-portal-${p.hub_id}`}>
      <span className="min-w-0 flex-1 flex items-center gap-2">
        <span className="text-[13px] font-medium truncate" title={t("connectors.hubId", { id: p.hub_id })}>
          {p.name}
        </span>
        {p.default && <span className={TAG_ACCENT}>{t("common.default")}</span>}
        {p.sandbox && <span className={TAG_WARN}>{t("connectors.sandbox")}</span>}
        {p.access && (
          <span className={TAG_QUIET} data-testid={`hubspot-access-tag-${p.hub_id}`}>
            {p.access === "write" ? t("connectors.readWrite") : t("connectors.readOnly")}
          </span>
        )}
        {!p.managed && <span className={TAG_QUIET}>{t("connectors.privateApp")}</span>}
      </span>
      {!p.default && (
        <button
      ...
      <button
        className={XBTN}
        title={t("connectors.disconnectPortal")}
    ...
    <>
      ...
      <div className={GRP}>
        <div className={ROW} data-testid="hubspot-hidden-fields">
          <span className={LABEL}>{t("connectors.hiddenFields")}</span>
          <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
            ...
            <input
              ...
              placeholder={t("connectors.propertyPlaceholder")}
              value={draft}
      ...
      <div className={FOOT}>{t("connectors.hiddenFieldsFootnote")}</div>
    </>
  );
}
