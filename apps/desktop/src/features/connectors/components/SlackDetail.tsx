  ...
  return (
    <div data-testid="slack-workspaces">
      <div className="flex items-center gap-3.5 mb-5">
        <ConnectorBadge connector={c} size={44} title={t("connectors.brand.slack")} />
        <div className="min-w-0 flex-1">
          <h2 className="text-[20px] font-semibold tracking-tight leading-tight">
            {t("connectors.brand.slack")}
          </h2>
              ...
              <>
                ...
                <span data-testid="slack-mode-badge">
                  {relay
                    ...
                    : t("connectors.connectedSocketMode")}
                </span>
              </>
            ) : (
      ...
      {c.connected && (
        <div className={FOOT + " mt-2"}>{t("connectors.namesFromSlack")}</div>
      )}
          ...
          <>
            <div className={ROW}>
              <span className="min-w-0 flex-1 text-[12.5px] text-muted flex items-center gap-2 flex-wrap">
                <span>{t("connectors.noOneAllowedSlack")}</span>
...
function DisconnectBtn({ teamId, busy, onClick }: { teamId: string; busy: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      disabled={busy}
    >
      {busy ? t("connectors.disconnecting") : t("connectors.disconnectWorkspace")}
    </button>
  );
}
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        ...
        {allowed.map((u) => (
          ...
          >
            ...
            {label(u)}
            {u === installerId && <span className="text-[10.5px] text-faint">{t("connectors.chipYou")}</span>}
            {protectedIds?.includes(u) ? (
              <span
                className="text-[10.5px] text-faint"
                title={t("connectors.removeOwnerTitle")}
              >
                {t("connectors.ownerTag")}
              </span>
            ) : (
...
function PersonPicker({
      ...
      getSlackDirectory(teamId || "default", q)
        .then((r) => {
          if (r.ok) {
            setRows(r.members || []);
            setErr(null);
          } else setErr(r.error || t("connectors.directoryUnavailable"));
        })
        .catch(() => setErr(t("connectors.directoryUnavailable")));
    }, 200);
  ...
  const pick = async (m: SlackMember) => {
    ...
    if (result?.ok === false) {
      setErr(result.error || t("connectors.couldNotAddPerson"));
      return;
    }
    setOpen(false);
    setQ("");
            ...
            {err ? (
              ...
              candidates.map((m) => (
                <button
                  ...
                  title={t("connectors.id", { value: m.id })}
                  onMouseDown={(e) => {
...
function ApprovalOwnersRow({
  ...
  const remove = async (userId: string) => {
    ...
    if (!result.ok) {
      setErr(result.error || t("connectors.couldNotRemoveOwner"));
      return;
    }
    setErr(null);
    onChanged();
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {owners.length === 0 && (
          <span className="text-[12px] text-warnInk">
            {t("connectors.chooseApprovalOwner")}
          </span>
        )}
        {owners.map((u) => (
          <span
          ...
          >
            ...
            {label(u)}
            {u === installerId && <span className="text-[10.5px] text-faint">{t("connectors.installerTag")}</span>}
            {editable && (
              <button className={XBTN} title={t("connectors.removeApprovalOwner")} onClick={() => remove(u)}>
                ×
              </button>
            )}
          </span>
        ...
        {!editable && owners.length > 0 && (
          <span className="text-[11.5px] text-faint">{t("connectors.setByInstaller")}</span>
        )}
