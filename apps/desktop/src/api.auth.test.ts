...
it("desktop mode: sends no token header and connects WS without subprotocols, even if a stale token global exists", async () => {
  vi.stubGlobal("__DELTA_API_TOKEN__", "stale-launch-token");
  const request = vi.fn(async (_url: string, init?: RequestInit) => {
    expect(new Headers(init?.headers).get("X-Delta-Token")).toBeNull();
    return {
      json: async () => ({
        status: "ok",
...
// comes from the dev sources (vite define / VITE_DELTA_API_TOKEN / runtime override).
it("browser dev mode: authenticates REST and session WebSocket calls with the dev token", async () => {
  ...
  const request = vi.fn(async (_url: string, init?: RequestInit) => {
    expect(new Headers(init?.headers).get("X-Delta-Token")).toBe("launch-token");
    return {
      json: async () => ({
        status: "ok",
        default_workspace: null,
  ...
  const session = new Session("s1", "/workspace", "code", { onEvent: vi.fn() });
  const socket = (session as unknown as { ws: FakeWebSocket }).ws;
  expect(socket.protocols).toEqual(["delta", "launch-token"]);
});
