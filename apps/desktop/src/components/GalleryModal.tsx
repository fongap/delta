...
export function GalleryModal({
  ...
  const openDetail = async (slug: string) => {
    ...
    setDetail(d ?? { ok: false, error: t("gallery.errorLoadDetails") });
  };
  ...
  const install = async (slug: string) => {
    ...
    setBusy(false);
    if (!r.ok) {
      setMsg(r.error || t("gallery.errorInstall"));
      return;
    }
      ...
      <div className="flex items-center gap-2 mb-4">
        {(
          [
            ...
            ["openworker", "gallery.filterOpenWorker"],
            ["team", "gallery.filterTeam"],
          ] as [Source, string][]
        ).map(([key, label]) => (
          <button
            key={key}
          ...
          return (
            ...
            >
              ...
              <div className="shrink-0 flex items-center">
                {isInstalled ? (
                  <span className="text-[12px] text-muted">{t("common.installed")}</span>
                ) : (
  ...
  const detailView = detailSlug && (
    <div data-testid="gallery-detail">
      ...
      {!detail ? (
        ...
        <div className="text-[12.5px] text-danger">{detail.error || t("gallery.errorLoadDetails")}</div>
      ) : (
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="shrink-0">
              {installed.has(detailSlug) ? (
                <span className="text-[12.5px] text-muted">{t("common.installed")}</span>
              ) : (
            ...
            <div className={CARD + " p-4"} data-testid="gallery-capabilities">
              ...
              <div className="space-y-2 text-[12.5px]">
                <div>
                  ...
                  {caps.risk.length > 0 && (
                    <span className="text-faint">{t("gallery.risk", { risk: caps.risk.join(", ") })}</span>
                  )}
                </div>
                <div>
                  <span className="text-muted">{t("gallery.permissions")}</span>
                  {t("gallery.recommendedMode", { mode: caps.recommended_mode })}
                  {caps.messaging ? t("gallery.canMessaging") : ""}
                  {caps.mcp.length > 0 ? t("gallery.mcp", { mcp: caps.mcp.join(", ") }) : ""}
                </div>
                {(detail.recommends?.length ?? 0) > 0 && (
                  <div>
                    <div className="text-muted mb-1.5">{t("gallery.connectionsTitle")}</div>
                      ...
                      {detail.recommends!.map((r) => (
                        <div key={r.kind + r.ref} className="flex items-baseline gap-2">
                          <span className={CHIP + " inline-flex items-center gap-1 shrink-0"}>
                            ...
                            {r.ref}
                            {r.tier === "core" ? t("gallery.coreTier") : ""}
                          </span>
          ...
          {loading ? (
            ...
            <div className={CARD + " p-5 flex items-center gap-4"} data-testid="gallery-signin">
              ...
              <button className={BTN_ACCENT} onClick={signIn} disabled={signingIn}>
                {signingIn ? t("gallery.checkBrowser") : t("common.signIn")}
              </button>
            </div>
          ) : detailSlug ? (
            detailView
