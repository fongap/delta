...
function sidecarToken(): string {
  const state =
    ...
    (process.platform === "win32"
      ? join(process.env.APPDATA || homedir(), "delta")
      : join(homedir(), ".config", "delta"));
  try {
...
export async function scratchBaseIfReady(): Promise<string | null> {
  try {
    ...
    if (res.ok && s.model_ready) {
      return String(s.scratch_base || "~/Delta").replace(/^~(?=\/|$)/, homedir());
    }
  } catch {
    /* backend unreachable */
  }
...
export async function startDeltaSession(page: Page) {
  await page.goto("/");
...
export async function sendTask(page: Page, text: string) {
  await page.getByPlaceholder(/Ask Delta/).fill(text);
