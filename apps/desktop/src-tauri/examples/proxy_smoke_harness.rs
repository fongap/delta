...
fn main() -> std::io::Result<()> {
        ...
        for stream in sidecar.incoming() {
            ...
            std::thread::spawn(move || {
                ...
                if text.to_ascii_lowercase().contains("upgrade: websocket") {
                    let has_token = text
                        .to_ascii_lowercase()
                        .contains(&format!("sec-websocket-protocol: delta, {TOKEN}\r\n"));
                    let response = format!(
                    ...
                    let body = format!(
                        ...
                        text.to_ascii_lowercase()
                            .contains("x-delta-token: harness-token")
                    );
    ...
    let proxy_port = delta_desktop_lib::proxy_start_for_tests(sidecar_port, TOKEN.to_string())
        .expect("start proxy");
