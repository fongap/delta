...
def looks_like_unparsed_tool_call(
    text: str | None, tools: list[dict[str, Any]] | None = None
) -> bool:
    """True when assistant text still carries tool-call markup that salvage couldn't turn
    into a call — i.e. the model tried to call a tool and the syntax was mangled or cut
    off. Only meaningful when tools were actually offered. The engine uses it to end the
    turn as a retriable error instead of presenting the fragment as an answer."""
