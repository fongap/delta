"""Control-plane authority guard (ADR-012/015/016/020/022/023/024).
...
2. **Enforcement (opt-in)**: when ``DELTA_RUST_AUTHORITY=1`` AND this
   ...
   for direct artifact registration calls in non-test code and
...
def _scan_enforcement() -> list[tuple[Path, int, str, str]]:
    ...
    for search_dir in search_dirs:
        ...
        for py in search_dir.rglob("*.py"):
            ...
            # Idempotency, Ledger, and Task Identity are hard-cut (ADR-022/023/024):
