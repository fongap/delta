...
from core.engine import ApprovalOutcome, PermissionRequest, TurnEngine
...
def test_leaked_tool_call_ends_the_turn_as_a_retriable_error(tmp_path):
    """A tool call the endpoint couldn't parse must not pass as an answer. Ending
    "completed" made a half-written call indistinguishable from the model deciding it was
    done — the user saw narration trailing off into stray tags. It ends on the error path
    so the UI offers Retry; the drift is probabilistic, so retrying the same model usually
    works.
...
def test_ordinary_text_answer_still_completes(tmp_path):
    """Guard the other side: prose that merely mentions tool syntax inside code fences is
    a real answer and must still complete normally."""
    engine, _ = _engine(
        tmp_path,
