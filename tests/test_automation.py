...
def test_tz_local_and_unknown_resolve_to_none():
    """_tz returns None (naive = the machine's zone, DST resolved per fire date), never a
    frozen offset captured at compute time — and an unknown IANA name falls back to local."""
    from core.automation.store import _tz

    assert _tz("local") is None
    assert _tz("") is None
    assert _tz("Not/ARealZone") is None
    from zoneinfo import ZoneInfo

    assert _tz("UTC") == ZoneInfo("UTC")


@pytest.mark.skipif(
    not hasattr(time, "tzset"), reason="TZ env manipulation needs time.tzset (POSIX)"
)
