...
When a turn is interrupted at the wrong moment, the append-only JSONL can end up with
a user message between an assistant ``tool_calls`` block and its ``tool`` result.
Providers reject this ordering (Anthropic 400/2013, OpenAI "tool_call_ids did not have
response messages"), making the session permanently unrecoverable.

``ConversationStore._repair_tool_pairing`` reorders messages on load so every tool
result immediately follows its call, synthesising a placeholder when no result exists.

Semantic port of andrewyng/openworker 9e145d9ceb6268b6957fef6aa1889cb99e9f35be
(imports adapted coworker.* -> core.*).
"""
...
def test_dangling_call_gets_placeholder():
    """A tool call with no matching result gets a synthesised placeholder — but only
    when the thread has moved past the call (not a trailing pending call)."""
    messages = [
        _user("go"),
        _assistant_with_calls("c1"),
...
def test_trailing_pending_call_not_repaired():
    """A tool call as the last message is a pending/interrupted call — the engine will
    resume it, so we must not inject a placeholder."""
    messages = [
        _user("go"),
        _assistant_with_calls("c1"),
    ]
