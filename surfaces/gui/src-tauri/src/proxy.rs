...
//! token; it adds `X-OpenWorker-Token` to every forwarded REST request and rewrites the
//! WebSocket subprotocol (`["openworker", <token>]`) for every upgrade, so the sidecar's
//! own auth (coworker/server/app.py `require_sidecar_token` / `_websocket_authenticated`)
...
/// localhost dev/browser builds. Mirrors `_ALLOWED_ORIGIN_RE` in coworker/server/app.py.
...
async fn handle_connection(client: TcpStream, config: Arc<ProxyConfig>) {
    if let Err(error) = serve_connection(client, &config).await {
        ...
        eprintln!("[coworker-proxy] connection error: {error}");
    }
}
...
async fn relay_http(
    ...
    for (name, value) in &head.headers {
        if HOP_BY_HOP.contains(&name.as_str()) || name == "x-openworker-token" {
            continue;
        }
    ...
    forwarded.push_str(&format!("x-openworker-token: {}\r\n", config.token));
    forwarded.push_str("\r\n");
...
async fn relay_websocket(
    ...
    // subprotocol list for ["openworker", <token>] — what the sidecar's
