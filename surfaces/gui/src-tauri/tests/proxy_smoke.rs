...
async fn serve_rest_once(listener: TcpListener) {
    ...
    assert!(
        head.to_ascii_lowercase()
            .contains("x-openworker-token: smoke-test-token\r\n"),
...
async fn serve_ws_once(listener: TcpListener) {
    ...
    assert!(
        head.to_ascii_lowercase()
            .contains("sec-websocket-protocol: openworker, smoke-test-token\r\n"),
...
use openworker_desktop_lib::proxy_start_for_tests;
