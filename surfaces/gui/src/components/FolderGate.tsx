...
export function FolderGate({ onChoose, onCancel, create }: Props) {
  ...
  const open = async (p: string, doCreate = false) => {
    ...
    else setError(res.error || t("folder.errors.openFailed", undefined, "could not open that folder"));
  };
  ...
  return (
    <div className="gate-overlay">
      <div className="gate">
        ...
        <h2>{create ? t("folder.newProject", undefined, "New project") : t("folder.chooseTitle", undefined, "Choose a project folder")}</h2>
        ...
        <div className="gate-input">
          <input
            ...
            autoFocus
          />
          <button className="btn" onClick={browse} title={t("folder.pickFolder", undefined, "Pick a folder")}>
            {t("folder.browse", undefined, "Browse…")}
          </button>
          <button className="btn primary" onClick={() => open(path, create)} disabled={!path.trim()}>
            {create ? t("folder.create", undefined, "Create") : t("common.open", undefined, "Open")}
          </button>
        </div>
        ...
        {recents.length > 0 && (
          <>
            <div className="gate-label">{t("common.recent", undefined, "Recent")}</div>
