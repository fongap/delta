...
When @delta is tagged in a channel with no subscribed session, the router spawns a
delta session that OWNS that thread and replies into it. This store is the
...
class MentionSessionStore:
    ...
    def _save(self) -> None:
        if not self.path:
            return
        save_json_state(self.path, {"threads": [asdict(t) for t in self._threads]})
