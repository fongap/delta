...
from core.environment import environment_context
...
def _skill_dirs(workspace: Path | None) -> list[Path]:
    ...
    if workspace is not None:
        dirs.append(workspace / ".delta" / "skills")
    return dirs
...
def build_engine(
    *,
    ...
    if memory_store is not None:
        ...
        instructions = f"{instructions}\n\n{_MEMORY_GUIDANCE}"
        # What the delta KNOWS is fixed at session start (MEMORY-SPEC §7.1): a
    ...
    def context_provider() -> str:
        ...
        # knowledge, so it must bite the moment the user flips it. What the delta
