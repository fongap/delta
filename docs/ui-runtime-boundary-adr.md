...
- Status: Accepted
- Date: 2026-08-23
- Scope: the core session UI in `surfaces/gui`

## Context

Delta currently centralizes REST and WebSocket transport in `surfaces/gui/src/api.ts`,
...
in `coworker/server/contracts.py` and `surfaces/gui/src/runtime-contract.ts`, but:
