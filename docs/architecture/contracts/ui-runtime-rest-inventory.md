...
This document inventories the REST boundary used by `apps/desktop` in the current
...
- Production scope: `apps/desktop/src/**/*.{ts,tsx}`.
- Test-only direct calls are recorded separately at the end.
- Paths in the **Consumer modules** column are relative to `apps/desktop/src`.
...
- The wrapper attaches `X-Delta-Token` when an API token is available.
