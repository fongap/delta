...
> **R1.5 更新（2026-09-07）**：本 ADR 描述的"委托 `write_tasks` per-op subprocess"实现已被 P0-1 取代。R1.5 实际生产路径：delegate 改走统一 `delta_core` 进程（`DeltaCoreClient` → `delta_core` → SQLite），`write_tasks` 仅保留为诊断工具。详见 ADR-017 §R1.5 生产切权收口。
